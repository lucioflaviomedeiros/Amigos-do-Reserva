...
export default function FornecedorCadPage() {
  ...
  const [form, setForm]       = useState({
    name: '', type: 'autonomo', section: 'servicos',
    category: '', description: '', whatsapp: '',
    phone: '', instagram: '', website: '', city: '', email: ''
  })

  const set = (k: string, v: string) => setForm(f => ({ ...f, [k]: v }))

  async function submit() {
    if (!form.name || !form.whatsapp || !form.category) {
      alert('Preencha nome, categoria e WhatsApp')
      return
    }
    setLoading(true)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--cream)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'white', borderRadius: 20, width: '100%', maxWidth: 580, overflow: 'hidden', boxShadow: '0 8px 40px rgba(26,46,30,0.16)' }}>
          ...
          {done ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-mid)', lineHeight: 1.7 }}>
                Seu cadastro foi recebido e será analisado pelo administrador em até 48h.
              </p>
            </div>
          ) : (
            <>
              <div style={{ background: 'var(--gold-pale)', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: 'var(--text-mid)' }}>
                📋 Após enviar, seu cadastro será analisado e publicado em até 48h.
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text-mid)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>Descrição *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>{inp('Cidade / Região', 'city', 'Ex: Nova Lima e região')}</div>
              </div>
